...
public class AuthenticationHelper extends BaseAuthenticationProvider {
    ...
    private final String[] mScopes = {"User.Read", "User.ReadBasic.All", "Mail.Send", "Mail.Read"};

    private AuthenticationHelper(Context ctx, final IAuthenticationHelperCreatedListener listener) {
        PublicClientApplication.createSingleAccountPublicClientApplication(ctx, R.raw.msal_config, new IPublicClientApplication.ISingleAccountApplicationCreatedListener() {
            @Override
    ...
    public CompletableFuture<IAuthenticationResult> acquireTokenInteractively(Activity activity) {
        ...
        SignInParameters parameters = SignInParameters.builder().withActivity(activity).withScopes(Arrays.asList(mScopes)).withCallback(getAuthenticationCallback(future)).build();
    ...
    private AuthenticationCallback getAuthenticationCallback(CompletableFuture<IAuthenticationResult> future) {
    ...
    public CompletableFuture<String> getAuthorizationTokenAsync(@NonNull URL requestUrl) {
        if (shouldAuthenticateRequestWithUrl(requestUrl)) {
            return acquireTokenSilently().thenApply(IAuthenticationResult::getAccessToken);
        }
